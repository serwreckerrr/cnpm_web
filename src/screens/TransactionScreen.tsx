...
export function TransactionsScreen({
  ...
  const monthlyData = user?.isGuest
    ? []
    : [
  ...
  const monthlyBills = user?.isGuest
    ? []
    : [
        {
          id: '1',
  ...
  const recentTransactions = user?.isGuest
    ? []
    : [
        {
          id: '1',
            ...
            <div className="space-y-3">
              {monthlyBills.map((bill) => (
                ...
                >
                  ...
                  >
                    {bill.status === 'paid'
                      ? `${t('paid')} - ${bill.paidDate}`
                      : bill.status === 'pending'
                      ? `${t('pending')} - ${t('due')}: ${bill.dueDate}`
                      : t('failed')}
                  </div>
                </button>
              ))}
